       ...
       <?php 
        ...
        elseif(isset($module_id))
        {
            ...
            if(count($modules_actionss) > 0)
              ...
              <tr class="odd gradeX">
                <td>
                  <input type="checkbox" class="checkboxes" name="group_id[]" value="<?php echo $row['group_id'];?>"/>
                </td>
                <td><?php echo $row['group_name'];?></td>
                <td><?php echo $row['description'];?></td>
                <td><?php if($row['has_units']== 1):?> HAS UNITS <?php else:?> HAS NO UNITS <?php endif;?></td>
                <td>
                <?php
                                 if(isset($sub_module_id))
                                    {
                      ...
                      <a href="<?php echo $sub_module_id.'/'.$roww['sub_module_action_id'].'/'.$row['group_id'];?>" class="<?php if ($roww['sub_module_action_id'] == 'create'){
            ...
            ->where('user_type',$this->session->userdata('user_type'))
                       ...
                       <a href="<?php echo $module_id.'/'.$roww9['action_id'].'/'.$row['group_id'];?>"  class="<?php if ($roww9['action_id'] == 'create') {
             ...
             function deletee()
              {
                 var group_id = [];
                  ...
                  if(group_id ==''){
                    alert('SELECT ATLEAST ONE GROUP TO DELETE');
                  }else{
                 if(confirm('Are you sure ????'))
                {
                  
                  jQuery.post("Groups/delete",
                  {
                     group_id : group_id
                  },
