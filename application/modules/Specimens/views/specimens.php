       ...
       <?php 
        ...
        elseif(isset($module_id))
        {
            ...
            if(count($modules_actionss) > 0)
              ...
              <tbody>
              <?php foreach ($specimens as $row) {?>
              <tr class="odd gradeX">
                <td>
                  <input type="checkbox" class="checkboxes" name="specimen_id[]" value="<?php echo $row['specimen_id'];?>"/>
                </td>
                ...
                <?php
                      ...
                      <a href="<?php echo $sub_module_id.'/'.$roww['sub_module_action_id'].'/'.$row['specimen_id'];?>" class="<?php if ($roww['sub_module_action_id'] == 'create'){
            ...
            ->where('user_type',$this->session->userdata('user_type'))
                       ...
                       <a href="<?php echo $module_id.'/'.$roww9['action_id'].'/'.$row['specimen_id'];?>"  class="<?php if ($roww9['action_id'] == 'create') {
             ...
             function deletee()
              {
                 var specimen_id = [];
                  jQuery('.checkboxes:checked').each(function(i,e){
                    specimen_id.push(jQuery(this).val());
                  });
                  if(specimen_id ==''){
                    alert('SELECT ATLEAST ONE SPECIMEN TO DELETE');
                  }else{
                 if(confirm('Are you sure ????'))
                {
                  
                  jQuery.post("Specimens/delete",
                  {
